...
import { createChatMessage } from '../services/chatroom.api';
...
export default function ChatRoomPage() {
  ...
  const [details, setRoomDetails] = useState({ name: '', languague: '' });
  ...
  useEffect(() => {
    // TODO: set up polling for new messages
    setChats(PLACEHOLDER_ROOM_DATA.messages);
    setRoomDetails(PLACEHOLDER_ROOM_DETAILS);
  }, []);
  ...
  function getChatColorBySenderId(senderId) {
    const colors = [
      'pistachio',
      'jet',
      'pear',
    ...
    console.log('colorIdx', colors[colorIndex]);
    return colors[colorIndex];
  }

  const mappedChats = chats.map((chat) => ({
    ...chat,
    color: getChatColorBySenderId(chat.senderId),
  }));
  ...
  return (
    <DefaultPageLayout>
      ...
      <div className="h-[calc(100dvh_-_9rem)] p-1 bg-white text-black overflow-y-scroll">
        <div className="px-1">
          {chats.map((chat) => (
            <div key={chat._id} className="flex gap-2">
              <p
                className={`text-sm font-semibold text-nowrap text-${getChatColorBySenderId(chat.senderId)} brightness-90`}
                key={chat.id}
              >
                {chat.senderName}:
              </p>
